...
# To crate a network with this architeture we
# need to modefied the size of some weight matrices
# This is done in the get_weights from weight_init_Jan_aproach
nClasses = 10
nfilt = 3
...
# Crate the network
model = MELTNET_Jan_aproach(params_2, nClasses, state_2, false)

# Load the model
Flux.loadmodel!(model, BSON.load("MELTNET_Jan_aproach_100epoch_randomweight_lr0005.bson")[:model_state])
...
with_theme(theme_latexfonts()) do
    ...
    #CairoMakie.save("imagenes/comparativa_J_xl_NaN.png", f)
    f
end
...
with_theme(theme_latexfonts()) do
    ...
    #CairoMakie.save("imagenes/hist_J_norm_meltnet-nemo.png", f)
    f
end