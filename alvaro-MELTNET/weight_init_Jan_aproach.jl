function get_weights(params, nClasses, nfilt, init_method)
    ...
    for ii in [16, 19, 22]
        params["Offset"][ii] = Flux.zeros32(Int(nfb[3:end][ii-2]/2), 1)
        params["Scale" ][ii] = Flux.ones32( Int(nfb[3:end][ii-2]/2), 1)
    end
    ...
    for ii in 1:3 
        ...
        params["deWeights2"][ii] = init_method(nfilt, nfilt, Int(nf[5-ii]/2), nf[4-ii])
        params["deWeights3"][ii] = init_method(nfilt, nfilt,     nf[4-ii]   , nf[4-ii])
        params["deWeights4"][ii] = init_method(    1,     1, Int(nf[5-ii]/2), nf[4-ii])
...
function initialise_state(state)
    ...
    for ii in [16, 19, 22]
        state["batchnorm"][ii]["TrainedMean"]     = Flux.zeros32(Int(nf[ii]/2), 1)
        state["batchnorm"][ii]["TrainedVariance"] = Flux.ones32( Int(nf[ii]/2), 1)
    end
